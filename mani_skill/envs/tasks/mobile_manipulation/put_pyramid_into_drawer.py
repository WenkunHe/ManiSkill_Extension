...
@register_env(
    "MobilePutPyramidIntoDrawer-v1",
    asset_download_ids=["partnet_mobility_cabinet"],
    max_episode_steps=200,
)
class PutPyramidIntoDrawerEnv(BaseEnv):
    """
    **Task Description:**
    Use the Fetch mobile manipulation robot to put a pyramid into drawer.
    ...
    **Success Conditions:**
    - The drawer is open at most 2% of the way, and the pyramid is placed stably in the drawer.
    ...
    **Goal Specification:**
    ...
    def _after_reconfigure(self, options):
        ...
        #    if self.num_envs == 20:
        #        print(-collision_mesh.bounding_box.bounds)
        ...
        self.min_target_qpos = qmin + (qmax - qmin) * self.min_drawer_frac
        self.goal_delta_qpos = qmin + (qmax - qmin) * self.min_open_frac * 0.5
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            #qs = torch.zeros((b, 4))
            #qs[:, 0] = 1
            self.pyramid.set_pose(Pose.create_from_pq(xy, qs))
    ...
    def _after_control_step(self):
        ...
        xy[:, 0] = xy[:, 0] + self.goal_delta_qpos
        xy[:, 2] = xy[:, 2] + 0.1
        self.goal.set_pose(Pose.create_from_pq(p=xy))
        if self.gpu_sim_enabled:
            self.scene._gpu_apply_all()

    def pyramid_reach_goal(self):
        handle_link_pos = self.handle_link_positions()
        pyramid_x_in_range = (handle_link_pos[:, 0] + 0.03) <= self.pyramid.pose.p[:, 0]
        pyramid_y_in_range = (handle_link_pos[:, 1] - 0.15 <= self.pyramid.pose.p[:, 1]) & (self.pyramid.pose.p[:, 1] <= handle_link_pos[:, 1] + 0.15)
        pyramid_z_in_range = (handle_link_pos[:, 2] - 0.08) <= self.pyramid.pose.p[:, 2]
        #pyramid_below_goal = (self.pyramid.pose.p[:, 2] - self.goal.pose.p[:, 2]) <= 0.08
        return pyramid_x_in_range & pyramid_y_in_range & pyramid_z_in_range
    
    def strict_pyramid_reach_goal(self):
        pyramid_to_goal_dist = torch.linalg.norm(
            self.goal.pose.p[:, :2] - self.pyramid.pose.p[:, :2], axis=1
        )
        pyramid_above_goal = -0.03 <= (self.pyramid.pose.p[:, 2] - self.goal.pose.p[:, 2])
        pyramid_below_goal = (self.pyramid.pose.p[:, 2] - self.goal.pose.p[:, 2]) <= 0.05
        return (pyramid_to_goal_dist <= 0.03) & pyramid_above_goal & pyramid_below_goal
    ...
    def evaluate(self):
        ...
        is_grasped = self.agent.is_grasping(self.pyramid, max_angle=100)
        is_released = self.agent._is_released(self.pyramid)

        if hasattr(self, 'success_stage1'):
            tcp_to_pyramid_dist = torch.linalg.norm(
                self.agent.tcp.pose.p - self.pyramid.pose.p, axis=1
            )
            self.success_stage1 = self.success_stage0 & (tcp_to_pyramid_dist <= 0.03)
        
        if hasattr(self, 'success_stage2'):
            self.success_stage2 = open_small & is_released & self.pyramid_reach_goal()

        if hasattr(self, 'success_stage3'):
            self.success_stage3 = is_released & self.pyramid_reach_goal() & (amount_to_open_left > 0.98) & link_is_static
    ...
    def _get_obs_extra(self, info: Dict):
        obs = dict(
            ...
            tcp_to_pyramid_pose=self.pyramid.pose.p - self.agent.tcp.pose.p,
            pyramid_to_goal_pose = self.goal.pose.p - self.pyramid.pose.p,
        ...
        if hasattr(self, 'success_stage0'):
            tcp_to_pyramid_dist = torch.linalg.norm(
                self.pyramid.pose.p - self.agent.tcp.pose.p, axis=1
            )
            pyramid_to_goal_dist = torch.linalg.norm(
                self.goal.pose.p - self.pyramid.pose.p, axis=1
            )
            tcp_reach_pyramid = self.success_stage0 & (tcp_to_pyramid_dist <= 0.03)
            pyramid_reach_goal = self.success_stage0 & self.strict_pyramid_reach_goal()
        else:
            tcp_reach_pyramid = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
            pyramid_reach_goal = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)

        obs.update(
            stage=one_hot_stage,
            tcp_reach_pyramid=tcp_reach_pyramid,
            pyramid_reach_goal=pyramid_reach_goal,
        )
        return obs
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        reward += self.success_stage0 * (1 - torch.tanh(2.4 * tcp_to_pyramid_dist)) * 3
        ...
        reward += is_grasped * (1 - torch.tanh(5.0 * pyramid_to_goal_dist)) * 5

        agent_finger_dist = self.agent._get_finger_dist()
        pyramid_to_goal_dist = torch.linalg.norm(
            self.goal.pose.p[:2] - self.pyramid.pose.p[:2], axis=1
        )
        pyramid_reach_goal = self.strict_pyramid_reach_goal()
        reward[pyramid_reach_goal] = 23.0
        l_force, r_force = self.agent._get_finger_force(self.pyramid)
        reward += pyramid_reach_goal * (1 - torch.tanh(0.1 * (l_force + r_force))) * 5

        #if self.num_envs == 16:
        #    print('tcp pose', self.agent.tcp.pose.p)
        #    print('is_grasped', is_grasped)
        #    print('extra_reward', is_grasped * (1 - torch.tanh(5.0 * pyramid_to_goal_dist)) * 5)
        #    print('manip stage', self.manip_stage)

        reward[self.success_stage2] = 30.0
        reward += (is_released & self.pyramid_reach_goal()) * (reaching_reward + 2 * amount_to_open_left) * 2
