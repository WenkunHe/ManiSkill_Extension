...
@register_env(
    "MobilePutCubeIntoDrawer-v1",
    asset_download_ids=["partnet_mobility_cabinet"],
    max_episode_steps=200,
)
class PutCubeIntoDrawerEnv(BaseEnv):
    """
    **Task Description:**
    Use the Fetch mobile manipulation robot to put a rubber cube into drawer.
    ...
    **Success Conditions:**
    - The drawer is open at most 2% of the way, and the cube is placed stably in the drawer.
    ...
    **Goal Specification:**
    ...
    def cube_reach_goal(self):
        ...
        cube_x_in_range = (handle_link_pos[:, 0] + 0.03) <= self.cube.pose.p[:, 0]
        cube_y_in_range = (handle_link_pos[:, 1] - 0.15 <= self.cube.pose.p[:, 1]) & (self.cube.pose.p[:, 1] <= handle_link_pos[:, 1] + 0.15)
        cube_z_in_range = (handle_link_pos[:, 2] - 0.08) <= self.cube.pose.p[:, 2]
        #cube_below_goal = (self.cube.pose.p[:, 2] - self.goal.pose.p[:, 2]) <= 0.08
        return cube_x_in_range & cube_y_in_range & cube_z_in_range
    ...
    def strict_cube_reach_goal(self):
        ...
        cube_below_goal = (self.cube.pose.p[:, 2] - self.goal.pose.p[:, 2]) <= 0.05
        return (cube_to_goal_dist <= 0.03) & cube_above_goal & cube_below_goal
