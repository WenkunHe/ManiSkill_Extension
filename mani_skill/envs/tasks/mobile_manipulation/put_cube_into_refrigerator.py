...
@register_env(
    "MobilePutCubeIntoRefrigerator-v1",
    asset_download_ids=["partnet_mobility_cabinet"],
    max_episode_steps=100,
)
class PutCubeIntoRefrigeratorEnv(BaseEnv):
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            qs = torch.zeros((b, 4))
            qs[:, 0] = 1
            self.cube.set_pose(Pose.create_from_pq(xy, qs))
    ...
    def cube_reach_goal(self):
        #handle_link_pos = self.handle_link_positions()
        cube_x_in_range = (-0.15 <= self.cube.pose.p[:, 0]) & (self.cube.pose.p[:, 0] <= 0.15)
        cube_y_in_range = (-0.18 <= self.cube.pose.p[:, 1]) & (self.cube.pose.p[:, 1] <= 0.18)
        cube_above_goal = (self.cube.pose.p[:, 2] - self.goal.pose.p[:, 2]) >= -0.15
        #cube_z_in_range = (handle_link_pos[:, 2] - 0.08) <= self.cube.pose.p[:, 2]
        #cube_below_goal = (self.cube.pose.p[:, 2] - self.goal.pose.p[:, 2]) <= 0.08
        return cube_x_in_range & cube_y_in_range & cube_above_goal
    
    def strict_cube_reach_goal(self):
        cube_to_goal_dist = torch.linalg.norm(
            self.goal.pose.p[:, :2] - self.cube.pose.p[:, :2], axis=1
        )
        #cube_x_in_range = (self.goal.pose.p[:, 0] - 0.05) <= self.cube.pose.p[:, 0]
        #cube_y_in_range = (-0.05 <= self.cube.pose.p[:, 1]) & (self.cube.pose.p[:, 1] <= 0.05)
        cube_xy_in_range = (cube_to_goal_dist <= 0.06)
        cube_above_goal = -0.03 <= (self.cube.pose.p[:, 2] - self.goal.pose.p[:, 2])
        cube_below_goal = (self.cube.pose.p[:, 2] - self.goal.pose.p[:, 2]) <= 0.03
        return cube_xy_in_range & cube_above_goal & cube_below_goal
    ...
    def evaluate(self):
        ...
        is_grasped = self.agent.is_grasping(self.cube, max_angle=100)
        is_released = self.agent._is_released(self.cube)

        if hasattr(self, 'success_stage1'):
            tcp_to_cube_dist = torch.linalg.norm(
                self.agent.tcp.pose.p - self.cube.pose.p, axis=1
            )
            self.success_stage1 = self.success_stage0 & (tcp_to_cube_dist <= 0.03)
        
        if hasattr(self, 'success_stage2'):
            self.success_stage2 = open_small & is_released & self.cube_reach_goal()

        if hasattr(self, 'success_stage3'):
            self.success_stage3 = is_released & self.cube_reach_goal() & (amount_to_open_left > 0.98) & link_is_static
    ...
    def _get_obs_extra(self, info: Dict):
        obs = dict(
            ...
            tcp_to_cube_pose=self.cube.pose.p - self.agent.tcp.pose.p,
            cube_to_goal_pose = self.goal.pose.p - self.cube.pose.p,
        ...
        if hasattr(self, 'success_stage0'):
            ...
            tcp_reach_cube = self.success_stage0 & (tcp_to_cube_dist <= 0.03)
            cube_reach_goal = self.success_stage0 & self.strict_cube_reach_goal()
        else:
            tcp_reach_cube = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
            cube_reach_goal = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)

        obs.update(
            stage=one_hot_stage,
            tcp_reach_cube=tcp_reach_cube,
            cube_reach_goal=cube_reach_goal,
        )
        return obs
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        if self.num_envs == 20:
            with open("sac_output_refrigerator_success_aa.txt", "a") as file:
                ...
                file.write(f"Cube: {self.cube.pose.p}\n")
                file.write(f"Finger Dist: {self.agent._get_finger_dist()}\n")
                file.write(f"Cube Reach Goal: {self.cube_reach_goal()}\n")
                file.write(f"Tcp Cube Dist: {torch.linalg.norm(self.agent.tcp.pose.p[:, :2] - self.cube.pose.p[:, :2], axis=1)}\n")
                file.write(f"Horizonx: {self.goal.pose.p[:, 0] - self.cube.pose.p[:, 0]}\n")
                file.write(f"Horizony: {self.goal.pose.p[:, 1] - self.cube.pose.p[:, 1]}\n")
                file.write(f"Vertical: {self.goal.pose.p[:, 2] - self.cube.pose.p[:, 2]}\n")
                l_force, r_force = self.agent._get_finger_force(self.cube)
        ...
        reward += self.success_stage0 * (1 - torch.tanh(2.4 * tcp_to_cube_dist)) * 3
        ...
        reward += is_grasped * (1 - torch.tanh(5.0 * cube_to_goal_dist)) * 5

        agent_finger_dist = self.agent._get_finger_dist()
        cube_to_goal_dist = torch.linalg.norm(
            self.goal.pose.p[:2] - self.cube.pose.p[:2], axis=1
        )
        cube_reach_goal = self.strict_cube_reach_goal()
        reward[cube_reach_goal] = 23.0
        l_force, r_force = self.agent._get_finger_force(self.cube)
        reward += cube_reach_goal * (1 - torch.tanh(0.1 * (l_force + r_force))) * 5

        #if self.num_envs == 16:
        #    print('tcp pose', self.agent.tcp.pose.p)
        #    print('is_grasped', is_grasped)
        #    print('extra_reward', is_grasped * (1 - torch.tanh(5.0 * cube_to_goal_dist)) * 5)
        #    print('manip stage', self.manip_stage)

        reward[self.success_stage2] = 30.0
        reward += (is_released & self.cube_reach_goal()) * (reaching_reward + 2 * amount_to_open_left) * 2
