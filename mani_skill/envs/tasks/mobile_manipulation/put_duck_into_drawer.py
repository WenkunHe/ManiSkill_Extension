...
@register_env(
    "MobilePutDuckIntoDrawer-v1",
    asset_download_ids=["partnet_mobility_cabinet"],
    max_episode_steps=200,
)
class PutDuckIntoDrawerEnv(BaseEnv):
    """
    **Task Description:**
    Use the Fetch mobile manipulation robot to put a rubber duck into drawer.
    ...
    **Success Conditions:**
    - The drawer is open at most 2% of the way, and the duck is placed stably in the drawer.
    ...
    **Goal Specification:**
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            #qs = torch.zeros((b, 4))
            #qs[:, 0] = 1
            self.duck.set_pose(Pose.create_from_pq(xy, qs))
    ...
    def duck_reach_goal(self):
        handle_link_pos = self.handle_link_positions()
        duck_x_in_range = (handle_link_pos[:, 0] + 0.03) <= self.duck.pose.p[:, 0]
        duck_y_in_range = (handle_link_pos[:, 1] - 0.15 <= self.duck.pose.p[:, 1]) & (self.duck.pose.p[:, 1] <= handle_link_pos[:, 1] + 0.15)
        duck_z_in_range = (handle_link_pos[:, 2] - 0.08) <= self.duck.pose.p[:, 2]
        #duck_below_goal = (self.duck.pose.p[:, 2] - self.goal.pose.p[:, 2]) <= 0.08
        return duck_x_in_range & duck_y_in_range & duck_z_in_range
    
    def strict_duck_reach_goal(self):
        duck_to_goal_dist = torch.linalg.norm(
            self.goal.pose.p[:, :2] - self.duck.pose.p[:, :2], axis=1
        )
        duck_above_goal = -0.03 <= (self.duck.pose.p[:, 2] - self.goal.pose.p[:, 2])
        duck_below_goal = (self.duck.pose.p[:, 2] - self.goal.pose.p[:, 2]) <= 0.05
        return (duck_to_goal_dist <= 0.03) & duck_above_goal & duck_below_goal
    ...
    def evaluate(self):
        ...
        is_grasped = self.agent.is_grasping(self.duck, max_angle=100)
        is_released = self.agent._is_released(self.duck)

        if hasattr(self, 'success_stage1'):
            tcp_to_duck_dist = torch.linalg.norm(
                self.agent.tcp.pose.p - self.duck.pose.p, axis=1
            )
            self.success_stage1 = self.success_stage0 & (tcp_to_duck_dist <= 0.03)
        
        if hasattr(self, 'success_stage2'):
            self.success_stage2 = open_small & is_released & self.duck_reach_goal()

        if hasattr(self, 'success_stage3'):
            self.success_stage3 = is_released & self.duck_reach_goal() & (amount_to_open_left > 0.98) & link_is_static
    ...
    def _get_obs_extra(self, info: Dict):
        obs = dict(
            ...
            tcp_to_duck_pose=self.duck.pose.p - self.agent.tcp.pose.p,
            duck_to_goal_pose = self.goal.pose.p - self.duck.pose.p,
        ...
        if hasattr(self, 'success_stage0'):
            tcp_to_duck_dist = torch.linalg.norm(
                self.duck.pose.p - self.agent.tcp.pose.p, axis=1
            )
            duck_to_goal_dist = torch.linalg.norm(
                self.goal.pose.p - self.duck.pose.p, axis=1
            )
            tcp_reach_duck = self.success_stage0 & (tcp_to_duck_dist <= 0.03)
            duck_reach_goal = self.success_stage0 & self.strict_duck_reach_goal()
        else:
            tcp_reach_duck = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
            duck_reach_goal = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)

        obs.update(
            stage=one_hot_stage,
            tcp_reach_duck=tcp_reach_duck,
            duck_reach_goal=duck_reach_goal,
        )
        return obs
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        reward += self.success_stage0 * (1 - torch.tanh(2.4 * tcp_to_duck_dist)) * 3
        ...
        reward += is_grasped * (1 - torch.tanh(5.0 * duck_to_goal_dist)) * 5

        agent_finger_dist = self.agent._get_finger_dist()
        duck_to_goal_dist = torch.linalg.norm(
            self.goal.pose.p[:2] - self.duck.pose.p[:2], axis=1
        )
        duck_reach_goal = self.strict_duck_reach_goal()
        reward[duck_reach_goal] = 23.0
        l_force, r_force = self.agent._get_finger_force(self.duck)
        reward += duck_reach_goal * (1 - torch.tanh(0.1 * (l_force + r_force))) * 5

        #if self.num_envs == 16:
        #    print('tcp pose', self.agent.tcp.pose.p)
        #    print('is_grasped', is_grasped)
        #    print('extra_reward', is_grasped * (1 - torch.tanh(5.0 * duck_to_goal_dist)) * 5)
        #    print('manip stage', self.manip_stage)

        reward[self.success_stage2] = 30.0
        reward += (is_released & self.duck_reach_goal()) * (reaching_reward + 2 * amount_to_open_left) * 2
