    ...
    **Goal Specification:**
    ...
    def _get_obs_extra(self, info: Dict):
        ...
        if "state" in self.obs_mode:
            ...
            #print('tcp_to_handle_pos', obs['tcp_to_handle_pos'].shape)
            #print('target_link_qpos', obs['target_link_qpos'].shape)
            #print('target_handle_pos', obs['target_handle_pos'].shape)
        
        seq_len, device = obs['tcp_pose'].shape[0], obs['tcp_pose'].device
        obs.update(stage=torch.zeros((seq_len), device=device))
        return obs
...
@register_env("OpenRefrigerator-v1", max_episode_steps=100)
class OpenRefrigeratorEnv(OpenCabinetDrawerEnv):
    TRAIN_JSON = (
